...
if (in_array("--limit", process.argv)) {
  context_length_limit =
    process.argv[parseInt(process.argv.indexOf("--limit")) + 1];
}

let navigation_timeout = 10000;
if (in_array("--timeout", process.argv)) {
  navigation_timeout = parseInt(
    process.argv[parseInt(process.argv.indexOf("--timeout")) + 1]
  );
}
...
if (in_array("--headless", process.argv)) {
  headless =
    (process.argv[parseInt(process.argv.indexOf("--headless")) + 1] ??
      "true") !== "false";
}
